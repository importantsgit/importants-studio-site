#!/usr/bin/env python3
"""posts/*.md 를 /log/ 목록과 /log/<slug>/ 글 페이지로 낸다.
...
def render(posts_dir="posts", out_dir="log",
           post_tpl="post.template.html", list_tpl="log.template.html"):
    ...
    for idx, m in enumerate(metas):
        ...
        nav = []
        if older:
            nav.append('지난 글: <a href="/log/%s/">%s</a>' % (esc(older["slug"]), esc(older["title"])))
        if newer:
            nav.append('다음 글: <a href="/log/%s/">%s</a>' % (esc(newer["slug"]), esc(newer["title"])))
        nav.append('<a href="/log/">목록으로</a>')
...
if __name__ == "__main__":
    ms = render()
    print("글 %d편, /log/ 와 /log/<slug>/ 생성" % len(ms))
